namespace Dreamteck.Forever
{
    ...
    public partial class LevelSegment : MonoBehaviour
    {
        ...
        Matrix4x4 extrudeMatrix = new Matrix4x4();
        Quaternion extrudeRotation = Quaternion.identity;
        public event EmptyHandler onExtruded;
        ...
        /// </summary>
        public virtual void OnBeforeExtrude()
        {
            rootMatrix = transform.localToWorldMatrix;
            inverseRootMatrix = rootMatrix.inverse;
            if (type == Type.Custom)
            {
                for (int i = 0; i < customPaths.Length; i++) customPaths[i].Transform();
            }
            for (int i = 0; i < objectProperties.Length; i++) objectProperties[i].RuntimeInitialize();
            Evaluate(0.0, extrudeResult);
            _generationState = GenerationState.Extruding;
            if(type == Type.Custom)
            {
        ...
        /// <summary>
        /// Called by the SegmentExtruder on the main thread or on a separate thread to performa extrusion calculations
        /// </summary>
        public void Extrude()
        {
            if (type == Type.Custom)
            {
                for (int i = 0; i < customPaths.Length; i++) customPaths[i].CalculateSamples();
                return;
            } else path.CalculateSamples();
            
            //Make the first result the same as the previous one to prevent seams (stitch)
            if (stitch && previous != null)
            {
                path.samples[0].CopyFrom(previous.path.samples[previous.path.samples.Length - 1]);
                path.samples[0].percent = 0.0;
            }
            for (int i = 0; i < objectProperties.Length; i++) ExtrudeObject(objectProperties[i]);
        ...
        public void ExtrudeObject(ObjectProperty p)
        {
            ...
            p.targetRotation = TransformUtility.GetRotation(ref p.localToWorldMatrix);
            if (p.extrusionSettings.applyRotation)
            {
                Quaternion rot = extrudeRotation;
                if (p.extrusionSettings.keepUpright) rot = GetUprightRotation(extrudeRotation, p.extrusionSettings.upVector);
                if (p.isRoot) p.targetRotation = rot;
                else p.targetRotation = rot * p.localRotation;
            }
            ...
            Matrix4x4 normalMatrix = toWorldMatrix.transpose.inverse;
            ...
            if (p.extrusionSettings.bendMesh)
            {
                ...
                for (int i = 0; i < p.extrusionMesh.vertexCount; i++)
                {
                    ...
                    Vector3 worldNormal = extrudeRotation * p.extrusionMesh.normals[i];
        ...
        /// </summary>
        public virtual void OnPostExtrude()
        {
            //Apply bending properties
            if (type == Type.Extruded)
            {
                for (int i = 0; i < objectProperties.Length; i++) objectProperties[i].RuntimeApply();
                for (int i = 0; i < customPaths.Length; i++) customPaths[i].InverseTransform();
                _extruded = true;
            }
            _generationState = GenerationState.Idle;
            for (int i = 0; i < builders.Length; i++)
            {
                if (builders[i].enabled && builders[i].gameObject.activeInHierarchy && builders[i].queue == Builder.Queue.OnGenerate) builders[i].StartBuild(this);
            }
            if (onExtruded != null) onExtruded();
        }
