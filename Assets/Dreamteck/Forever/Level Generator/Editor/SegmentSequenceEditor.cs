namespace Dreamteck.Forever
{
    ...
    public class SegmentSequenceEditor
    {
        ...
        SegmentSequenceSettingsWindow sequenceSettingsWindow = null;

        List<SegmentSequence> sequenceAddress = new List<SegmentSequence>();
        ...
        public void DrawEditor()
        {
            ...
            if (sequenceAddress.Count == 0)
            {
                for (int i = 0; i < sequences.Length; i++) SequenceUI(sequences[i]);
        ...
        void SequenceUI(SegmentSequence sequence)
        {
            ...
            if (sequence.type == SegmentSequence.Type.Random) thumbSize.y += 16f;
            ...
            if (newObj != null) newSegment = new SegmentDefinition(newObj);
        ...
        GameObject SegmentField(Rect rect)
        {
            if (rect.Contains(Event.current.mousePosition))
            {
                Object obj = null;
                bool hasDrag = GetDraggedObject(out obj);
                if (hasDrag)
                {
                    Object dragObj = DragAndDrop.objectReferences[0];
                    if (dragObj != null && dragObj is GameObject && ((GameObject)dragObj).GetComponent<LevelSegment>())
                    {
                        GUI.color = new Color(1f, 1f, 1f, 0.5f);
                        GUI.Box(rect, "+", dragBox);
                        GUI.color = Color.white;
                        if (obj != null) return (GameObject)obj;
                    }
                }
            }
            return null;
        ...
        public static bool GetDraggedObject(out Object dragged)
        {
            dragged = null;
