...
namespace Dreamteck.Forever
{
    public class RandomPathGenerator : HighLevelPathGenerator
    {
        ...
        protected override void GeneratePoint(ref Point point, int pointIndex)
        {
            if (isFirstPoint)
            {
                ...
                if (useYaw && orientation.y == targetAngle.y)
                {
                    ...
                    turnRate.y = Random.Range(minTurnRate.y, maxTurnRate.y);
                }
                if (usePitch && orientation.x == targetAngle.x)
                {
                    ...
                    turnRate.x = Random.Range(minTurnRate.x, maxTurnRate.x);
                }
                if (useRoll && orientation.z == targetAngle.z)
                {
                    ...
                    turnRate.z = Random.Range(minTurnRate.z, maxTurnRate.z);
                }
            ...
            if (useColors) point.color = Color.Lerp(minColor.Evaluate(progress), maxColor.Evaluate(progress), Random.Range(0f, 1f));
            if (useSizes) point.size = Mathf.Lerp(minSize.Evaluate(progress), maxSize.Evaluate(progress), Random.Range(0f, 1f));
        }
        ...
        protected override void OnPostGeneration(SplinePoint[] points)
        {
            ...
            if (minSegmentOffset != Vector3.zero || maxSegmentOffset != Vector3.zero) OffsetPoints(points, Vector3.Lerp(minSegmentOffset, maxSegmentOffset, Random.Range(0f, 1f)), segmentOffsetSpace);
            if (isNewLevel && (newLevelMinOffset != Vector3.zero || newLevelMaxOffset != Vector3.zero)) OffsetPoints(points, Vector3.Lerp(newLevelMinOffset, newLevelMaxOffset, Random.Range(0f, 1f)), levelOffsetSpace);
        }

        public override void Initialize(LevelGenerator input)
        {
            base.Initialize(input);
            turnRate = new Vector3(Random.Range(minTurnRate.x, maxTurnRate.x), Random.Range(minTurnRate.y, maxTurnRate.y), Random.Range(minTurnRate.z, maxTurnRate.z));
            targetAngle = orientation;
            if (useStartYawTarget) targetAngle.y = startTargetOrientation.y;
            if (useStartPitchTarget) targetAngle.x = startTargetOrientation.x;
            if (useStartRollTarget) targetAngle.z = startTargetOrientation.z;
        }
